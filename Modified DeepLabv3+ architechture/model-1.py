...
def SqueezeNet(input_shape):
    ...
    x1 = Conv2D(filters=24, kernel_size=7, strides=2, padding='same', use_bias=False, activation='relu')(input_shape)
...
def ASPP(image_features):
    ...
    y_6 = Conv2D(filters=8, kernel_size=3, padding='same', dilation_rate = 6,use_bias=False)(image_features)
    y_6 = BatchNormalization(name=f'bn_3')(y_6)
    y_6 = Activation('relu', name=f'relu_3')(y_6)

    y_12 = Conv2D(filters=8, kernel_size=1, padding='same', dilation_rate = 12,use_bias=False)(image_features)
...
def DeepLabV3Plus(inputs, classes=1):
    ...
    x = Conv2D(filters=8, kernel_size=3, padding='same', use_bias=False)(x)
